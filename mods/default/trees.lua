...
minetest.register_lbm({
	...
	nodenames = {"default:sapling", "default:willow_sapling",
...
function default.grow_jungle_tree(pos, bad)
	--[[
		NOTE: willow tree-placing code is currently duplicated in the engine
		and in games that have saplings; both are deprecated but not
		replaced yet
	--]]
	if bad then
		error("Deprecated use of default.grow_willow_tree")
	end
	...
	local c_air = minetest.get_content_id("air")
	local c_ignore = minetest.get_content_id("ignore")
	local c_jungletree = minetest.get_content_id("default:willow_tree")
	local c_jungleleaves = minetest.get_content_id("default:willow_leaves")
...
function default.grow_new_willow_tree(pos)
	local path = minetest.get_modpath("default") ..
		"/schematics/willow_tree_from_sapling.mts"
	minetest.place_schematic({x = pos.x - 2, y = pos.y - 1, z = pos.z - 2},
		path, "random", nil, false)
end
